...
export function InvoicesDataTable<TData extends Invoice, TValue>({
  ...
  const selectedIds = useMemo(() => Object.keys(rowSelection).map((k) => Number(k)), [rowSelection]);
  const selectedCount = selectedIds.length;

  useEffect(() => {
    if (searchInput === (filters.search ?? "")) return;
    const handler = setTimeout(() => onFilterChange("search", searchInput || undefined), 500);
      ...
      <div className="flex flex-col items-center justify-center py-20">
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>
              Vous n'avez aucune facture pour le moment ou les filtres appliqués n'ont retourné aucun résultat.
            </EmptyDescription>
          </EmptyHeader>
          ...
          {selectedCount > 0 && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {onBulkPending && (
                  <DropdownMenuItem onClick={() => onBulkPending(selectedIds)}>
                    Marquer en attente
                  </DropdownMenuItem>
                )}
                {onBulkPaid && (
                  <DropdownMenuItem onClick={() => onBulkPaid(selectedIds)}>
                    Marquer payée
                  </DropdownMenuItem>
                )}
              </DropdownMenuContent>
