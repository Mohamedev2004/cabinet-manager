...
export function SyncedLyricCaptions({
  ...
  useEffect(() => {
    ...
    setDuration(mediaRef.current?.duration || fallbackDuration);
  }, [fallbackDuration]);

  // Audio element lifecycle (when using audioSrc without video)
  useEffect(() => {
  ...
  }, [audioSrc, videoSrc, fallbackDuration, volume, playbackRate]);

  // Video element listeners (when using videoSrc)
  useEffect(() => {
    if (!videoSrc) return;
    ...
    const handleLoaded = () =>
      setDuration(
        Number.isFinite(video.duration) ? video.duration : fallbackDuration
      );
    const handleEnded = () => setIsPlaying(false);
    const handlePause = () => setIsPlaying(false);
    const handlePlay = () => setIsPlaying(true);
  ...
  const handlePlayPause = async () => {
    ...
    if (media) {
      ...
      if (media.ended || currentTime >= duration) {
        media.currentTime = 0;
        setCurrentTime(0);
      }
      await media.play();
  ...
  return (
    <div className="w-full">
      <div className="rounded-2xl border border-border/60 bg-background/70 p-4 shadow-lg backdrop-blur-xl sm:p-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              <span>
                {videoSrc ? "Video synced" : audioSrc ? "Audio synced" : "Timer synced"} ·{" "}
        ...
        {/* Media Display or Captions */}
        {videoSrc ? (
          <div className="relative overflow-hidden rounded-xl border border-border/60 bg-background">
            <div className="aspect-[16/9] w-full">
              <video
                ref={(el) => {
                  // assign to shared media ref
                  mediaRef.current = el as HTMLVideoElement | null;
                }}
                src={videoSrc}
                poster={poster}
